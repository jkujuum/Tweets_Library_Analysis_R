...
In this project, we would like to understand what people's attitude to libraries and what do they say about libraries in 2017 (from 01-01-2017 to 12-31-2017). We used key twitter search terms "library", "libraries", "librarian", "librarians", "librarianship", and scrapped all tweets from twitter. Our tweets dataset contains 97199 observations. 
...
data <- fread("tweets_2017_sentiment.csv", stringsAsFactors = FALSE, header= TRUE)
dim(data)
names(data); 
object.size(data) # 
```
...
d.1 <- d[1:150,]
wordcloud(d.1$word, d.1$freq, scale=c(5,.5),  # make a word cloud
...
write.csv(data2, "2017_tm_freq.csv", row.names = FALSE)
```
...
```{r,eval=T,echo=FALSE,results='hide'}
### Splitting data
data2 <- fread("2017_tm_freq.csv")  #dim(data2)
names(data2)[1:20] 
dim(data2)  
...
Prediction Error is small (0.026), so the model is acceptable. 
```{r,eval=T,echo=FALSE,results='hide'}
...
People in 2017 thinks libraries are wonderful, awesome, excellent and etc. 
```{r,eval=T,echo=FALSE}
...
## Sentiment vs Month
Below is a graph of people's sentiment to library during months. We see fluctuation from January to December. After statistical analysis we found all numbers random about the mean, which mean the fluctuation of sentiment is just due to randomness. (See Excel document (sentiment_graph_2017) for specific statistical analysis). The mean of sentiment of 2017 is 0.13. In general, the overall sentiment of tweets is positive. 

```{r,eval=T,echo=FALSE}
knitr::include_graphics("sentiment2017.png")
```
...
From the following pie chart, we can tell 46.46% people have postitive feeling about library, 43.12% have neutural feeling and 10.42% have negative feelings.

```{r,eval=T,echo=FALSE}
knitr::include_graphics("pie2017.png")
```
