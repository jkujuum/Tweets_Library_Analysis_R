...
In this project, we would like to understand what people's attitude to libraries and what do they say about libraries in 2015 (from 01-01-2015 to 12-31-2015). We used key twitter search terms "library", "libraries", "librarian", "librarians", "librarianship", and scrapped all tweets from twitter. Our tweets dataset contains 97199 observations. 
...
data <- fread("tweets_2015_sentiment.csv", stringsAsFactors = FALSE, header= TRUE)
dim(data)
names(data); 
object.size(data) # 
```
...
The most common words in 2015 tweets about libraries are shown below
```{r,eval=T,echo=FALSE}
...
write.csv(data2, "2015_tm_freq.csv", row.names = FALSE)
```
...
```{r,eval=T,echo=FALSE,results='hide'}
### Splitting data
data2 <- fread("2015_tm_freq.csv")  #dim(data2)
names(data2)[1:20] 
dim(data2)  
...
Below is the wordcloud of all the ppositive words from all 2015 tweets. The larger the word is, the more positive it correlates to sentiment. 
People in 2015 thinks libraries are perfect, excellent, greatest and etc. 
```{r,eval=T,echo=FALSE}
...
Below is the wordcloud of all the negative words from all 2015 tweets. The larger the word is, the more negative it correlates to sentiment. 
People in 2015 thinks libraries are bad, sad and crazy. Word "cold" could imply the air conditioning or hear in libraries are not good. "Little" and "small" may imply that people thinks libraries are small and have little books. "Behind" may implies libraries may be a little outdated. We can dig into each word and figure out how could libraries improve themselves.  

```{r,eval=T,echo=FALSE}
wordcloud(bad.word[1:150], bad.fre[1:150],scale=c(3.8,.3),  
...
## Sentiment vs Month
Below is a graph of people's sentiment to library during months. We see fluctuation from January to December. After statistical analysis we found all numbers random about the mean, which mean the fluctuation of sentiment is just due to randomness. (See Excel document (sentiment_graph_2015) for specific statistical analysis). The mean of sentiment of 2017 is 0.119. In general, the overall sentiment of tweets is positive. 

```{r,eval=T,echo=FALSE}
knitr::include_graphics("sentiment2015.jpg")
```
...
From the following pie chart, we can tell 41.21% people have postitive feeling about library, 49.31% have neutural feeling and 9.66% have negative feelings.

```{r,eval=T,echo=FALSE}
knitr::include_graphics("pie2015.jpg")
```
